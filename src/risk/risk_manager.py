...
class RiskManager:
    ...
    def calculate_trade_parameters(self, equity: float, price: float, direction: str, ohlc_data: pd.DataFrame) -> Tuple[float, float, float]:
        ...
        try:
            ...
            if ideal_sl == 0 or abs(price - ideal_sl) < self.symbol_info.point * 2:
                self.log.error(f"La distance du Stop Loss est invalide ou trop serrée ({ideal_sl}). Le trade est annulé.")
                return 0.0, 0.0, 0.0
            ...
            self.log.error(f"Erreur de configuration ou de données lors du calcul des paramètres de trade : {e}", exc_info=True)
            return 0.0, 0.0, 0.0
        except Exception as e:
    ...
    def _calculate_initial_sl_tp(self, price: float, direction: str, ohlc_data: pd.DataFrame) -> Tuple[float, float]:
        rm_settings = self._config.get('risk_management', {})
        strategy = rm_settings.get('sl_tp_strategy', 'ATR_MULTIPLE')
        
        if strategy == "ATR_MULTIPLE":
            atr_settings = rm_settings.get('atr_settings', {})
            symbol_settings = atr_settings.get(self._symbol, atr_settings.get('default', {}))
