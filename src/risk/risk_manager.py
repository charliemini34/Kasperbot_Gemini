...
# Version: 18.0.0 (Structural-SL)
# Dépendances: MetaTrader5, pandas, numpy, logging, pytz, src.constants
# Description: Implémente la stratégie de SL structurel SMC avec fallback ATR.
...
class RiskManager:
    """
    Gère le risque avec SL/TP configurables (ATR, SMC Structure, SMC Liquidité).
    v18.0.0: Ajout de la stratégie SL 'SMC_STRUCTURE' avec fallback ATR.
    """
    ...
    def calculate_trade_parameters(self, equity: float, price: float, ohlc_data: pd.DataFrame, trade_signal: dict) -> Tuple[float, float, float]:
        try:
            ...
            # Calcul SL et TP selon stratégies configurées
    ...
    def _calculate_volume(self, equity: float, risk_percent: float, entry_price: float, sl_price: float) -> float:
        # ... (Logique inchangée, toujours basée sur sl_price fourni) ...
        self.log.debug("--- DÉBUT CALCUL VOLUME ---")
        risk_amount = equity * risk_percent
        sl_distance = abs(entry_price - sl_price)
        if sl_distance < self.point * 2: # Min distance pour éviter div par zéro
            self.log.error("Distance SL trop faible.")
            return 0.0
    ...
    def _calculate_initial_sl_tp(self, price: float, ohlc_data: pd.DataFrame, trade_signal: dict) -> Tuple[float, float]:
        ...
        tp_strategy = self.risk_settings.get('sl_tp_strategy', 'ATR_MULTIPLE') # Renommé pour clarté
        atr = self.calculate_atr(ohlc_data, self.risk_settings.get('atr_settings', {}).get('default', {}).get('period', 14))
        sl_calculated_structurally = False # Flag
        ...
        if self.sl_strategy == "SMC_STRUCTURE":
            ...
            if sl_structure_price and atr and atr > 0:
                ...
                # Validation du SL structurel
                min_sl_dist = atr * 0.5 # Ex: SL doit être au moins à 0.5 ATR
                if abs(price - potential_sl) >= min_sl_dist:
                    # Vérifier si le volume est viable avec ce SL
                    ...
                    if temp_volume >= self.symbol_info.volume_min:
                        ...
                        self.log.warning(f"SL Structurel ({potential_sl:.{self.digits}f}) rend volume ({temp_volume:.4f}) < min ({self.symbol_info.volume_min}). Fallback sur ATR.")
                else:
                    self.log.warning(f"SL Structurel ({potential_sl:.{self.digits}f}) trop proche du prix ({price:.{self.digits}f}). Fallback sur ATR.")
            else:
                 self.log.warning("Stratégie SL Structurelle choisie mais prix structurel manquant/invalide ou ATR invalide. Fallback sur ATR.")

        # Si SL structurel échoue ou si ATR est choisi
        if not sl_calculated_structurally:
            if atr is None or atr <= 0:
                self.log.error("ATR invalide. Impossible de calculer SL.")
                return 0.0, 0.0
        ...
        if not tp_calculated: # Fallback ou stratégie ATR
            if atr is None or atr <= 0: # Re-vérifie ATR si besoin
                self.log.error("ATR invalide. Impossible de calculer TP.")
                return sl, 0.0 # Retourne SL calculé, TP invalide
    ...
    def get_conversion_rate(self, from_currency: str, to_currency: str) -> Optional[float]:
        ...
        self.log.error(f"Impossible trouver conversion {from_currency} -> {to_currency}")
        return None
