...
class MT5Executor:
    ...
    def get_open_positions(self, symbol: str, magic: int = 0) -> list:
        ...
        AMÉLIORATION : Filtre par code MAGIC si fourni.
        """
        positions = self._mt5.positions_get(symbol=symbol)
        if positions is None:
            return []
        
        # Si un code magic est spécifié (différent de 0), on filtre.
        if magic > 0:
            return [pos for pos in positions if pos.magic == magic]
        
        # Sinon, on retourne toutes les positions sur le symbole (ancien comportement).
    ...
    def execute_trade(self, account_info, risk_manager, symbol, direction, score, raw_scores):
        ...
        if not price_info:
            self.log.error(f"Impossible d'obtenir le tick pour le symbole {symbol}. Ordre annulé.")
            return
        ...
        # Récupération du code MAGIC depuis la configuration via le risk_manager
        ...
        if volume > 0:
            ...
            if result:
                self._analyzer.record_trade_entry(
                    ticket=result.order,
                    symbol=symbol,
                    direction=direction,
                    volume=volume,
    ...
    def place_order(self, symbol, order_type, volume, price, sl, tp, magic_number):
        ...
        request = {
            ...
            "tp": tp,
            "deviation": 20,
            "magic": magic_number,  # Inclusion du code MAGIC dans la requête
            "comment": "Trade by KasperBot v3.3",
    ...
    def get_daily_pnl(self):
        """Calcule le PnL pour la journée en cours."""
        try:
            ...
            if history:
                ...
                # On ne somme que les deals de sortie (OUT) pour ne pas compter les entrées
    ...
    def check_for_closed_trades(self):
        """Logique à implémenter pour une analyse post-trade."""
        pass
        
    def get_newly_closed_trades_count(self):
        """Retourne le nombre de trades fermés depuis le dernier check."""
