...
class MT5Executor:
    ...
    v9.5 : Ajout d'un filtre de ratio Risque/Récompense (R/R).
    """
    ...
    def execute_trade(self, account_info, risk_manager, symbol, direction, ohlc_data, pattern_name, magic_number, market_trend, volatility_atr):
        """Orchestre le placement d'un trade, avec validation du R/R, et enregistre son contexte."""
        ...
        # --- NOUVELLE LOGIQUE : FILTRE DE RATIO RISQUE/RÉCOMPENSE ---
        risk_distance = abs(price - sl)
        reward_distance = abs(tp - price)

        if risk_distance == 0:
            self.log.warning(f"Trade sur {symbol} annulé : Distance de risque est nulle (SL = prix d'entrée).")
            return

        actual_ratio = reward_distance / risk_distance
        min_rr_ratio = risk_manager._config.get('risk_management', {}).get('min_rr_ratio', 0)

        if actual_ratio < min_rr_ratio:
            self.log.warning(f"Trade sur {symbol} annulé : Ratio R/R ({actual_ratio:.2f}) insuffisant. Minimum requis : {min_rr_ratio:.2f}.")
            return
        ...
        if volume > 0:
            self.log.info(f"Préparation de l'ordre {direction} {volume} lot(s) de {symbol} @ {price:.5f} (R/R: {actual_ratio:.2f})")
            ...
            if result:
                self._open_trades_context[result.order] = {
                    'symbol': symbol, 'direction': direction, 'open_time': datetime.utcnow().isoformat(),
                    'pattern_trigger': pattern_name, 'market_trend': market_trend, 'volatility_atr': volatility_atr
                }
        else:
    ...
    def check_for_closed_trades(self, magic_number):
        ...
        for ticket in closed_tickets:
            ...
            if history_deals:
                ...
                if exit_deal:
                    context = self._open_trades_context[ticket]
                    trade_record = {
                    ...
                    if self._archive_trade(trade_record):
                        successfully_archived.append(ticket)
                else:
                    self.log.warning(f"Trade #{ticket} fermé mais le deal de sortie est introuvable pour le moment. Réessai au prochain cycle.")
            else:
                self.log.warning(f"Trade #{ticket} fermé mais l'historique est indisponible pour le moment. Réessai au prochain cycle.")
        
        for ticket in successfully_archived:
            self._open_trades_context.pop(ticket, None)

    def _archive_trade(self, trade_record: dict) -> bool:
        try:
            df = pd.DataFrame([trade_record])
            file_exists = os.path.exists(self.history_file)
            df.to_csv(self.history_file, mode='a', header=not file_exists, index=False)
            self.log.info(f"Trade #{trade_record['ticket']} archivé avec succès (PnL: {trade_record['pnl']:.2f}$).")
            return True
        except Exception as e:
            self.log.error(f"Impossible d'archiver le trade #{trade_record['ticket']} dans le fichier CSV: {e}")
            return False
