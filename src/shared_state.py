# Fichier: main.py
# Version: 19.0.6 (Patch-UI-Init-Fix)
# Dépendances: MetaTrader5, pytz, PyYAML, Flask, playsound, time, threading, logging, webbrowser, os, datetime, decimal
# Description: Corrige P3.1 (UI vide) en initialisant l'état des symboles au démarrage.
...
BOT_VERSION = "v19.0.6-patch" # UI-Init-Fix
# --- FIN MODIFICATION ---
...
def analyze_and_trade_symbol(symbol: str, state: SharedState, config: dict, connector: MT5Connector, executor: MT5Executor, account_info, is_first_cycle: bool):
    ...
    try:
        ...
        if ohlc_data is None or ohlc_data.empty or len(ohlc_data) < 50:
            ...
            state.update_symbol_patterns(symbol, {}) # Mettre à jour avec des données vides si échec
            return
        ...
        # Mettre à jour l'état pour l'UI avec les patterns trouvés (ou absence de patterns)
    ...
    try:
        ...
        while not state.is_shutdown():
            try:
                ...
                # --- MODIFICATION P3.1.B : Initialiser l'état des symboles pour l'UI ---
                # Garantit que l'UI affiche les symboles surveillés dès le démarrage.
                if symbols_to_trade:
