...
def setup_logging(state: SharedState):
    ...
    for handler in [ui_handler, file_handler, console_handler]: handler.setFormatter(log_formatter)
    root_logger = logging.getLogger()
    root_logger.setLevel(logging.INFO)
    if not root_logger.handlers: [root_logger.addHandler(h) for h in [ui_handler, file_handler, console_handler]]
...
def load_yaml(filepath: str) -> dict:
    ...
    try:
        ...
        logging.critical(f"FATAL: Fichier de configuration '{filepath}' introuvable."); exit()
    return {}

def main_trading_loop(state: SharedState):
    """Boucle principale du bot v9.1, stable et sécurisée."""
        ...
        try:
            ...
            if all_bot_positions:
                for pos in all_bot_positions:
                    try:
                        ...
                        if tick: rm_pos.manage_open_positions([pos], tick)
            ...
            for symbol in symbols_to_trade:
                ...
                try:
                    ...
                    logging.error(f"Init RiskManager échouée pour {symbol}: {e}. Symbole invalide ?")
                    continue
                ...
                if trade_signal:
                    ...
                    if config['trading_settings']['live_trading_enabled']:
                        # --- CORRECTION : Ajout de l'argument 'magic_number' manquant ---
    ...
    connector.disconnect(); logging.info("Boucle de trading terminée.")
