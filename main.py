# Fichier: main.py
# Version: 17.0.4 (Sound-Alerts + DryRun)
# Dépendances: MetaTrader5, pytz, PyYAML, Flask, playsound
# Description: Ajoute mode dry-run explicite.
...
def play_alert_sound(config: dict):
    ...
    if sound_config.get('enabled', False):
        ...
        if os.path.exists(sound_file):
            ...
            except Exception as e: logging.error(f"Impossible de jouer le son '{sound_file}': {e}")
        else: logging.warning(f"Fichier son configuré '{sound_file}' introuvable.")

def main_trading_loop(state: SharedState):
    """Boucle principale qui orchestre le bot de trading."""
    logging.info("Démarrage de la boucle de trading v17.0.4 (Sound-Alerts + DryRun)...") # Version mise à jour
    ...
    while not state.is_shutdown():
        try:
            if not connector.check_connection():
                ...
                if not connector.connect(): time.sleep(20); continue
            ...
            if not account_info: logging.warning("Infos compte inaccessibles."); time.sleep(10); continue
            ...
            if symbols_to_trade:
                try:
                    ...
                    if limit_reached: state.update_status("Arrêt Urgence", "Limite perte jour atteinte.", is_emergency=True); time.sleep(60); continue
            ...
            for symbol in symbols_to_trade:
                try:
                    ...
                    if trade_signal and not is_first_cycle:
                        ...
                        if volume > 0:
                            # --- MODIFICATION: Ajout Condition Dry Run ---
                            if config['trading_settings']['live_trading_enabled']:
                                ...
                                logging.info(f"DRY RUN: Ordre {trade_signal['direction']} {volume:.{risk_manager.symbol_info.volume_digits}f} lots {symbol} @ {ohlc_data['close'].iloc[-1]:.{risk_manager.digits}f} (SL={sl:.{risk_manager.digits}f}, TP={tp:.{risk_manager.digits}f}) non envoyé.")
                            # --- FIN MODIFICATION ---
                        else:
            ...
            time.sleep(30)
        except KeyboardInterrupt: logging.info("Arrêt manuel (Ctrl+C)."); state.shutdown()
